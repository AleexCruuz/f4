...
// with DynaLoader and calls `f4_now_playing_get`. The app runs that
...
@_cdecl("f4_now_playing_get")
public func f4NowPlayingGet() {
    ...
    let queue = DispatchQueue(label: "com.f4.now-playing-adapter")
...
@_cdecl("f4_now_playing_watch")
public func f4NowPlayingWatch() {
    ...
    watching = true
    register(.main)
    let reader = DispatchQueue(label: "com.f4.now-playing-watch")
    ...
    func refresh() {
        pending?.cancel()
        let work = DispatchWorkItem { f4NowPlayingGet() }
        pending = work
