...
final class AppVolumeMixer: ObservableObject {
    ...
    private let buildQueue = DispatchQueue(label: "com.f4.utils.mixer", qos: .userInitiated)
    ...
    private let halQueue = DispatchQueue(label: "com.f4.utils.mixer.hal", qos: .userInitiated)
    ...
    private static func outputDevices(defaultUID: String?) -> [MixerOutputDevice] {
        ...
        for deviceID in deviceIDs {
            ...
            let name = read(deviceID, kAudioObjectPropertyName, &nameRef)
                ? nameRef as String
                : uid
            guard name != "F4 Mixer" else { continue }
...
private final class TapGainEngine: GainEngine {
    ...
    init?(objects: [AudioObjectID], gain: Float, outputDeviceUID: String) {
        ...
        let aggregate: [String: Any] = [
            kAudioAggregateDeviceNameKey: "F4 Mixer",
    ...
    private static let rateQueue = DispatchQueue(label: "com.f4.utils.mixer.rate",
                                                 qos: .userInitiated)
    ...
    private static let teardownQueue: OperationQueue = {
        ...
        queue.name = "com.f4.utils.mixer.teardown"
