...
final class NotchService: ObservableObject {
    ...
    private let menuSpaceQueue = DispatchQueue(label: "com.f4.notch-menu-space", qos: .utility)
    ...
    func hover(_ entered: Bool) {
        ...
        if inside {
            ...
            hoverWork = work
            DispatchQueue.main.asyncAfter(deadline: .now() + (expanded ? NotchQuickAccessLayout.hoverExitDelay : 0.04), execute: work)
        }
    }
