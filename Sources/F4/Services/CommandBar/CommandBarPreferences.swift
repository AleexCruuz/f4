...
enum CommandBarSource: String, CaseIterable, Identifiable {
    /// What F4 itself can do. Always on: it is what the bar is for.
    case actions
    case apps
    case menus
    case windows
    ...
    case uninstallApps
    case settingsPages
    /// The Mac's own Settings panes, which are not F4's and can be
...
enum CommandBarPreferences {
    ...
    ///
    /// What this Mac holds (the apps, F4's own actions, the Settings
