...
enum CommandBarCatalog {
    ...
    /// showing its own shortcut. This is how the bar reaches past F4
    ...
    /// runs, so one row serves both "gh" and "gh f4".
    ...
    private static func keepOnShelf(_ text: String) {
        let board = NSPasteboard(name: NSPasteboard.Name("com.f4.commandbar.selection"))
        board.clearContents()
