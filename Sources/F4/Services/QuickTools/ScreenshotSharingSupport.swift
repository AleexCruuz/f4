...
enum ScreenshotSharingSupport {
    /// Off: F4 runs no share server, and `productionEndpoint` sits on a
    /// reserved `.invalid` host that never resolves. Every share control
    /// hides and no upload starts until the fork runs its own.
    static let isAvailable = false
    static let productionEndpoint = URL(string: "https://screenshots.f4.invalid")!
    static let developerBundleIdentifier = "com.f4.utils.dev"
