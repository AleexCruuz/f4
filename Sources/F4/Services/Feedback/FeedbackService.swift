...
final class FeedbackService {
    ...
    /// Off: F4 runs no feedback server, and the endpoint below sits on a
    /// reserved `.invalid` host. No report is sent and no entry point shows
    /// until the fork runs its own.
    nonisolated static let isAvailable = false

    private let endpoint = URL(string: "https://screenshots.f4.invalid/v1/feedback")!
