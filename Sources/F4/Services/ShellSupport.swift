...
enum AdminShell {
    // F4 is a menu-bar agent (LSUIElement), so it is rarely the active
...
enum Sudoers {
    static let rulePath = "/etc/sudoers.d/f4-clamshell"
    ...
    private static let legacyRulePaths = [
        "/etc/sudoers.d/f4-utils-clamshell",
    ...
    private static let sleepStateQueue = DispatchQueue(label: "com.f4.utils.pmset-state")
