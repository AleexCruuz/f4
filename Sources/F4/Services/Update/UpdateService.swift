...
final class UpdateService: ObservableObject {
    ...
    /// Off: installs are verified against upstream's signing team, so no F4
    /// build could ever pass. The app never checks, downloads or installs
    /// until F4 signs its releases with a team of its own.
    static let isAvailable = false

    private let repository = "AleexCruuz/f4"
    ...
    func check(manual: Bool) {
        ...
        request.setValue("F4/\(AppInfo.version)", forHTTPHeaderField: "User-Agent")
            ...
            delegate = try BoundedUpdateDownloadDelegate(
                ...
                completion: { [weak self] tempURL, response, error in
                    ...
                    let dmgURL = FileManager.default.temporaryDirectory
                        .appendingPathComponent("F4-update.dmg")
    ...
    private func launchUserInstaller(appPath: String, dmgPath: String, pid: Int32,
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("f4-update-\(pid)-\(UUID().uuidString).sh")
        do {
...
final class BoundedUpdateDownloadDelegate: NSObject, URLSessionDataDelegate {
    ...
    init(byteLimit: Int64,
        ...
        let temporaryFileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("F4-update-\(UUID().uuidString).download")
