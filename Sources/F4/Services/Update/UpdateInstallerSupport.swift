...
enum UpdateInstallerSupport {
    ...
    static func installerScript() -> String {
        ...
        if [ -z "$SRC" ]; then
            ...
            # showing up in search as "F4.app.update-new".
            ...
            if /usr/bin/ditto "$SRC" "$STAGE"; then
                ...
                if [ "$BUNDLE_VERSION" = "$EXPECTED_VERSION" ]; then
                    ...
                    VERIFY_REQ='identifier "com.f4.utils" and anchor apple generic and certificate leaf[subject.OU] = "3D485NHW29"'
                    note fail-verify
    ...
    static func elevatedInstallCommand(appPath: String,
        ...
        return DetachedProcess.detachedShellCommand(
            quotedArgv: "/bin/sh -c \(script) f4-installer \(args)")
    }
