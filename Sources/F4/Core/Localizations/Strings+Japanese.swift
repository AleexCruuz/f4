...
extension Strings {
    static let ja = Strings(
        statusIdleTooltip: "F4：通常のスリープ",
        statusActiveUntil: "F4：次の時刻までスリープしない",
        statusActiveIndefinite: "F4：解除するまでスリープしない",
        ...
        notifyBatteryTitle: "F4 をオフにしました",
        notifyBatteryBody: "バッテリー残量が少なくなりました。充電を保護するため通常のスリープに戻しました。",
        adminPromptClamshellOn: "ふたを閉じても続けるには、F4 にパスワードが必要です。",
        adminPromptClamshellOff: "Mac の通常のスリープを元に戻すにはパスワードが必要です。",
        adminPromptRecover: "Mac のスリープを無効化したまま F4 が終了しました。通常のスリープに戻すにはパスワードを入力してください。",
        adminPromptUpdate: "アップデートをインストールするには F4 にパスワードが必要です。",
        adminPromptSudoersInstall: "ふたを閉じたモードをパスワードなしで切り替えられるよう、F4 が制限付きルール（pmset disablesleep のみ）を作成します。パスワードが必要なのはこの一度きりです。",
        adminPromptSudoersRemove: "パスワード不要のふたを閉じたモードのルールを F4 が削除します。",
        ...
        advancedResetDescription: "F4 に許可したすべてのアクセス権（アクセシビリティ、画面収録、フルディスクアクセスなど）、ログイン項目、ふたを閉じたモードのルールを削除します。最初からやり直すときや、アンインストールの前に便利です。アプリはインストールされたまま残ります。",
        ...
        advancedUninstallDescription: "上記をすべて実行したうえで、環境設定を削除し、F4 をゴミ箱に移動して、何も残しません。完了後にアプリは終了します。いつでも再インストールできます。",
        advancedUninstallButton: "F4 を完全にアンインストール",
        advancedUninstallConfirmTitle: "F4 をアンインストールしますか？",
        advancedUninstallConfirmBody: "F4 はアクセス権を消去し、環境設定を削除し、ゴミ箱に移動してから終了します。この操作はアプリからは取り消せませんが、ゴミ箱を空にするまではゴミ箱に残ります。",
        advancedUninstallFailedTitle: "アンインストールを中止しました",
        advancedUninstallFailedBody: "F4 が変更したシステム設定（スリープ、ファンの回転数、マウス加速）を元に戻せませんでした。何も削除していません。もう一度試して、パスワードの入力を求められたら許可してください。",
        ...
        keepAwakeAutoStart: "F4起動時にスリープを防ぐ",
        ...
        switcherTakeOverSystemShortcutsCaption: "F4 のスイッチャーが動作中の間だけ、対応する macOS のアプリとウインドウのショートカットを無効にします。実行中のすべてのアプリに切り替えられます。",
        ...
        uninstallerFDAHint: "リストで F4 をオンにします。表示されない場合は + をクリックし、アプリケーションから F4 を選びます。アクセス権はアプリを再度開いた後に有効になります。",
        ...
        homebrewMissingBody: "F4 は公式 Homebrew インストーラを Terminal で開けます。Terminal に手順が表示され、必要ならパスワードを求められます。",
        ...
        homebrewShellSetupBody: "Homebrew はインストールされていますが、Terminal がまだ brew コマンドを見つけられない場合があります。F4 は設定コマンドを Terminal で開けます。",
        ...
        homebrewTerminalFallback: "この操作では管理者パスワードを求めるために Terminal が必要です。F4 はパスワードを取得しません。",
        ...
        updateNotifyTitle: "F4 のアップデート",
        updateInstallFailedBody: "アップデートはダウンロードされましたが、適用できませんでした。GitHub のリリースページから最新版をダウンロードし、現在のアプリの上にドラッグして置き換えてください。",
        updateNeedsApplicationsTitle: "F4 を「アプリケーション」に移動してください",
        updateNeedsApplicationsBody: "アプリが、ディスクイメージやシステムの一時的な場所など、アップデートできない場所から実行されています。F4 を「アプリケーション」フォルダにドラッグし、そこから開いてからもう一度お試しください。",
        ...
        secureInputHeldFormat: "%@ が保持しているため、F4 が代わりに入力できません。パスワード欄からフォーカスを外すか、アプリを終了すると解除されます。",
        secureInputUnattributed: "実行中のどのアプリも保持していません。ログアウトしてから再度ログインすると解除されます。",
        secureInputUnidentified: "保持しているアプリを特定できないため、F4 が代わりに入力できません。",
        ...
        cleaningNeedsAxBody: "キーボードを安全にロックするには、F4 にアクセシビリティのアクセス権が必要です。システム設定で許可して、もう一度お試しください。",
        ...
        donateHeading: "F4 の成長を支える",
        donateMessage: "F4 は無料で独立したアプリとして、わたしの空き時間に開発しています。金銭的に支援したい場合は、Buy Me a Coffee が開発の継続に直接役立ちます。",
        donateButton: "Buy Me a Coffee で支援する",
        donateThanks: "来てくれてありがとう。🖤",
        supportIntroTitle: "F4 の成長を支える",
        supportIntroMessage: "開発を金銭的に支援したい場合は、Buy Me a Coffee が唯一の窓口です。",
        supportIntroStarButton: "GitHub で F4 にスターを付ける",
        supportIntroStarMessage: "金銭的な支援は決して必要ではありません。GitHub のスターは、より多くの人が F4 を見つける助けになり、開発にも大きな力になります。",
        supportIntroCoffeeButton: "Buy Me a Coffee で支援する",
        supportIntroLaterButton: "今はしない",
        supportIntroDoneButton: "完了",
        discordIntroTitle: "F4のDiscordコミュニティが始まります",
        discordIntroMessage: "F4 コミュニティは新しく、まだ成長の途中です。早い段階から参加して他のユーザーと出会い、アプリを囲む心地よい場所を一緒につくりましょう。",
        ...
        communityIntroFollowButton: "Xで@f4をフォロー",
        homebrewOfficialIntroTitle: "Homebrew 公式カタログに登場",
        homebrewOfficialIntroMessage: "F4 を Homebrew 公式カタログから直接インストールできるようになりました。",
        ...
        showMenuBarIconCaption: "F4 のアイコンが消えた場合（メニューバーに空きがなくなると macOS がアイコンを隠すことがあり、ノッチのある Mac でよく起こります）、アプリケーションまたは Spotlight から F4 を開き直してください。アイコンが再作成され、それでも隠れている場合はこのウインドウが開きます。",
        ...
        menuBarIconManagerHintFormat: "%@ が起動中で、アイコンを隠しセクションにしまっている可能性があります。そこで F4 を探すか、%@ で F4 を常に表示する設定にしてください。",
        ...
        cleanerNotifDenied: "F4 の通知がシステムでオフになっています。",
        cleanerNotifOpenSettings: "通知設定を開く…",
        launchAtLoginNeedsApplications: "ログイン時に開けない場所からアプリが実行されています。F4をアプリケーションフォルダにドラッグし、そこから開いてもう一度オンにしてください。",
        launchAtLoginNeedsApproval: "ログイン項目は登録されていますが、システム設定ではオフのままです。「システム設定 › 一般 › ログイン項目と機能拡張」を開き、「ログイン時に開く」でF4をオンにしてください。",
        ...
        mouseAccelerationCaption: "接続したマウスのポインタ加速を無効にします。オフにするか F4 を終了すると以前の設定に戻ります。",
