...
extension Strings {
    static let ru = Strings(
        statusIdleTooltip: "F4: обычный сон",
        statusActiveUntil: "F4: активно до",
        statusActiveIndefinite: "F4: активно без срока",
        menuEnableAwake: "Включить режим без сна",
        menuDisableAwake: "Выключить режим без сна",
        menuActivateFor: "Включить на…",
        menuSettings: "Настройки…",
        menuAbout: "О F4",
        menuQuit: "Выйти из F4",
        menuHide: "Скрыть F4",
        ...
        notifyBatteryTitle: "F4 отключён",
        notifyBatteryBody: "Низкий заряд. Обычный сон восстановлен, чтобы сохранить батарею.",
        adminPromptClamshellOn: "F4 нужен ваш пароль, чтобы Mac продолжал работать с закрытой крышкой.",
        adminPromptClamshellOff: "F4 нужен ваш пароль, чтобы вернуть обычный режим сна Mac.",
        adminPromptRecover: "F4 завершился, пока сон Mac был отключён. Введите пароль, чтобы вернуть обычный режим сна.",
        adminPromptUpdate: "F4 нужен ваш пароль, чтобы установить обновление.",
        adminPromptSudoersInstall: "F4 создаст ограниченное правило (только pmset disablesleep), чтобы переключать режим закрытой крышки без запроса пароля. Пароль понадобится только сейчас.",
        adminPromptSudoersRemove: "F4 удалит правило для режима закрытой крышки без пароля.",

        settingsTitle: "Настройки F4",
        tabGeneral: "Основное",
        tabEnergy: "Питание",
        ...
        advancedResetDescription: "Удаляет все разрешения, которые вы дали F4 (Универсальный доступ, Запись экрана, Полный доступ к диску и другие), элемент входа и правило закрытой крышки. Полезно, если хотите начать с чистого листа или перед удалением. Приложение останется установленным.",
        ...
        advancedUninstallDescription: "Сделает всё вышеперечисленное, затем удалит настройки и переместит F4 в Корзину без остатка. После завершения приложение закроется. Установить его снова можно в любой момент.",
        advancedUninstallButton: "Удалить F4 полностью",
        advancedUninstallConfirmTitle: "Удалить F4?",
        advancedUninstallConfirmBody: "F4 очистит разрешения, удалит настройки и переместится в Корзину, после чего закроется. Это нельзя отменить из приложения, но файл останется в Корзине, пока вы её не очистите.",
        advancedUninstallFailedTitle: "Удаление остановлено",
        advancedUninstallFailedBody: "F4 не смог вернуть изменённую системную настройку: режим сна, скорость вентиляторов или ускорение мыши. Ничего не удалено. Повторите попытку и разрешите запрос пароля, если он появится.",
        ...
        keepAwakeAutoStart: "Не давать уснуть при запуске F4",
        ...
        switcherEnable: "Использовать переключатель F4",
        ...
        switcherTakeOverSystemShortcutsCaption: "Отключает соответствующие сочетания macOS только на время работы переключателя F4. Все запущенные приложения остаются доступны.",
        ...
        uninstallerFDAHint: "Включите F4 в списке. Если его там нет, нажмите + и выберите F4 из папки «Программы». Доступ начнёт действовать только после повторного запуска приложения.",
        ...
        homebrewMissingBody: "F4 может открыть Терминал с официальным установщиком Homebrew. Терминал покажет шаги и при необходимости запросит ваш пароль.",
        ...
        homebrewShellSetupBody: "Homebrew установлен, но Терминал может ещё не видеть команду brew. F4 может открыть Терминал с нужной командой настройки.",
        ...
        homebrewTerminalFallback: "Для этой операции нужен Терминал, чтобы запросить пароль администратора. F4 не перехватывает пароли.",
        ...
        updateNotifyTitle: "Обновление F4",
        ...
        updateNeedsApplicationsTitle: "Переместите F4 в папку «Программы»",
        updateNeedsApplicationsBody: "Приложение запущено из места, которое нельзя обновить, например из образа диска или временного системного расположения. Перетащите F4 в папку «Программы», откройте его оттуда и попробуйте снова.",
        ...
        secureInputHeldFormat: "Его удерживает %@, поэтому F4 не может печатать за вас. Снимите фокус с поля пароля или завершите работу приложения, чтобы освободить его.",
        ...
        secureInputUnidentified: "Не удалось определить приложение, которое его удерживает, поэтому F4 не может печатать за вас.",
        ...
        cleaningNeedsAxBody: "Чтобы безопасно блокировать клавиатуру, F4 нужен доступ к Универсальному доступу. Выдайте его в Системных настройках и попробуйте снова.",
        ...
        donateHeading: "Помогите F4 развиваться",
        donateMessage: "F4 бесплатен, независим и создаётся в моё свободное время. Если вы хотите помочь финансово, Buy Me a Coffee напрямую поддерживает дальнейшую разработку.",
        donateButton: "Поддержать через Buy Me a Coffee",
        donateThanks: "Спасибо, что вы здесь. 🖤",
        supportIntroTitle: "Помогите F4 развиваться",
        supportIntroMessage: "Если вы хотите финансово поддержать разработку, сделать это можно только через Buy Me a Coffee.",
        supportIntroStarButton: "Поставить F4 звезду на GitHub",
        supportIntroStarMessage: "Финансовая поддержка совсем не обязательна. Звезда на GitHub помогает большему числу людей найти F4 и действительно влияет на его развитие.",
        supportIntroCoffeeButton: "Поддержать через Buy Me a Coffee",
        supportIntroLaterButton: "Не сейчас",
        supportIntroDoneButton: "Готово",
        discordIntroTitle: "Сообщество F4 в Discord только начинает расти",
        discordIntroMessage: "Сообщество F4 новое и всё ещё формируется. Присоединяйтесь сейчас, знакомьтесь с другими пользователями и помогите создать дружелюбное пространство вокруг приложения.",
        ...
        communityIntroFollowButton: "Подписаться на @f4 в X",
        homebrewOfficialIntroTitle: "Теперь в официальном каталоге Homebrew",
        homebrewOfficialIntroMessage: "Теперь F4 можно установить прямо из официального каталога Homebrew.",
        ...
        showMenuBarIconCaption: "Если значок F4 исчезнет (macOS может скрывать значки строки меню, когда в ней не хватает места, что часто бывает на Mac с вырезом), откройте F4 снова из папки «Программы» или через Spotlight: это пересоздаст значок и, если он всё ещё скрыт, откроет это окно.",
        ...
        menuBarIconManagerHintFormat: "%@ запущен и, возможно, держит значок в своей скрытой секции. Поищите F4 там или настройте %@ всегда показывать F4.",
        ...
        cleanerNotifDenied: "Уведомления F4 отключены в системе.",
        cleanerNotifOpenSettings: "Открыть настройки уведомлений…",
        launchAtLoginNeedsApplications: "Приложение запущено из места, из которого нельзя открываться при входе. Перетащите F4 в папку «Программы», откройте его оттуда и включите параметр снова.",
        launchAtLoginNeedsApproval: "Объект входа зарегистрирован, но в Системных настройках он выключен. Откройте «Системные настройки › Основные › Объекты входа и расширения» и включите F4 в разделе «Открывать при входе».",
        ...
        mouseAccelerationCaption: "Отключает ускорение указателя для подключённых мышей. Прежняя настройка вернётся после выключения функции или выхода из F4.",
