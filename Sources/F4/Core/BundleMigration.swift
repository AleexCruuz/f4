...
/// OLD PATH ("/Applications/F4 Utils.app"), so right after updating the
/// app is running from a bundle still named "F4 Utils.app". We rename
/// that bundle to "F4.app" through a detached helper that runs only after
...
///
/// When we are already running as "F4.app", we instead retire a stray
...
enum BundleMigration {
    private static let oldName = "F4 Utils.app"
    private static let newName = "F4.app"
    ...
    private static func renameSelfAndRelaunch(from old: URL) -> Bool {
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("f4-rename-\(pid)-\(UUID().uuidString).sh")
        do {
