...
enum AdminShell {
    // AltF4 is a menu-bar agent (LSUIElement), so it is rarely the active
...
enum Sudoers {
    static let rulePath = "/etc/sudoers.d/altf4-clamshell"
    ...
    private static let legacyRulePaths = [
        "/etc/sudoers.d/altf4-utils-clamshell",
    ...
    private static let sleepStateQueue = DispatchQueue(label: "com.altf4.utils.pmset-state")
