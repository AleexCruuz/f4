...
enum ScreenshotCapturePolicy {
    ...
    /// AltF4 windows" preference narrows what is left.
    ///
    ...
    static func excludedWindowIDs(hideAltF4Windows: Bool,
                                  ownWindowIDs: Set<CGWindowID>,
                                  protectedWindowIDs: Set<CGWindowID>) -> Set<CGWindowID> {
        hideAltF4Windows ? ownWindowIDs : ownWindowIDs.intersection(protectedWindowIDs)
    }

    static func canPickWindow(_ windowID: CGWindowID,
                              isOwnWindow: Bool,
                              hideAltF4Windows: Bool,
                              protectedWindowIDs: Set<CGWindowID>) -> Bool {
        !isOwnWindow
            || (!hideAltF4Windows && !protectedWindowIDs.contains(windowID))
    }
