...
final class NotchMusicService: ObservableObject {
    ...
    private let queue = DispatchQueue(label: "com.altf4.notch-music", qos: .utility)
    ...
    private static var adapter: [String]? {
        guard let script = Bundle.main.url(forResource: "now-playing", withExtension: "pl"),
              let library = Bundle.main.privateFrameworksURL?.appendingPathComponent("libAltF4NowPlaying.dylib"),
...
private final class NotchMusicPipeReader {
    private let queue = DispatchQueue(label: "com.altf4.notch-music-reader", qos: .utility)
