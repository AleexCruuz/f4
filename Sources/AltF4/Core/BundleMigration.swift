...
/// OLD PATH ("/Applications/AltF4 Utils.app"), so right after updating the
/// app is running from a bundle still named "AltF4 Utils.app". We rename
/// that bundle to "AltF4.app" through a detached helper that runs only after
...
///
/// When we are already running as "AltF4.app", we instead retire a stray
...
enum BundleMigration {
    private static let oldName = "AltF4 Utils.app"
    private static let newName = "AltF4.app"
    ...
    private static func renameSelfAndRelaunch(from old: URL) -> Bool {
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("altf4-rename-\(pid)-\(UUID().uuidString).sh")
        do {
