...
summary](https://github.com/altf4/altf4-utils/issues/838) sorts
...
AltF4 is one menu bar icon doing the job of a dozen paid Mac apps. Doing
...
is that every upstream breakage arrives here as a AltF4 bug, reported by
...
- **`./build.sh --test` is not proof that it compiles.** It builds a
  hand-written list of source files, and `Sources/AltF4/UI/` is largely
