...
MUTATIONS = [
    ("emoji family offers unsupported tones", "emoji", "Sources/AltF4/Services/CommandBar/CommandBarEmoji.swift",
    ...
    ("one-off emoji skips usage learning", "emoji", "Sources/AltF4/Services/CommandBar/CommandBarService.swift",
     "                self.recordUsage(of: entry)\n", "",
     "a one-off tone records exactly one use under the original emoji"),
    ("one-off emoji learns the action field instead of its search", "emoji", "Sources/AltF4/Services/CommandBar/CommandBarService.swift",
     "        case .argument, .actions:\n", "        case .argument:\n",
     "a one-off tone learns the search saved before opening actions"),
    ("output switches reuse another device's volume baseline", "core", "Sources/AltF4/Services/Notch/NotchService.swift",
    ...
    ("volume observation reads partially published controls", "core", "Sources/AltF4/Services/Notch/NotchService.swift",
     ".receive(on: DispatchQueue.main)\n            .sink { [weak self, weak mixer] _ in",
     ".sink { [weak self, weak mixer] _ in",
     "switching output never replaces its connection notice with stored volume or mute"),
    ("device alerts return to the fixed level width", "core", "Sources/AltF4/Services/Notch/NotchService.swift",
    ...
    ("device alert window ignores its content width", "core", "Sources/AltF4/Services/Notch/NotchService.swift",
    ...
    ("Nothing loses its music gate", "core", "Sources/AltF4/Services/Notch/NotchSupport.swift",
    ...
    ("resting music bypasses automatic opt-out", "core", "Sources/AltF4/Services/Notch/NotchSupport.swift",
    ...
    ("resting music retains a disabled reader", "core", "Sources/AltF4/Services/Notch/NotchService.swift",
     "            || NotchSupport.watchesMusicActivity())",
     "            || NotchSupport.idleContent() == .music || NotchSupport.watchesMusicActivity())",
     "disabled automatic music stops the reader even when resting content is Music"),
    ("closing music retains its on-demand reader", "core", "Sources/AltF4/Services/Notch/NotchService.swift",
     "        removeEventMonitors()\n        syncVisibleConsumers()\n    }\n\n    func toggle()",
     "        removeEventMonitors()\n    }\n\n    func toggle()",
     "closing manually opened controls stops the reader and never leaves a music strip behind"),
    ("a timed session hands over on one condition", "core", "Sources/AltF4/Services/KeepAwakeManager.swift",
    ...
    ("match mode labels grow back into sentences", "core", "Sources/AltF4/Core/KeepAwakeStrings.swift",
     "        matchAny: \"L\u2019une\",\n        matchAll: \"Toutes\",\n",
     "        matchAny: \"N\u2019importe quelle condition\",\n        matchAll: \"Toutes les conditions\",\n",
     "fr: the match mode labels fit the panel card"),
    ("recording metadata rebases after startup", "recording", "Sources/AltF4/Services/Recorder/RecorderSupport.swift",
    ...
    ("microphone returns to its changing native format", "recording", "Sources/AltF4/Services/Recorder/RecorderWriter.swift",
    ...
    ("system audio returns to its changing native format", "recording", "Sources/AltF4/Services/Recorder/RecorderWriter.swift",
     "let interleaved = Self.interleavedAudioSample(sampleBuffer, converter: &systemAudioConverter)",
     "let interleaved = Optional(sampleBuffer)",
     "the multitrack MOV survives an audio buffer-layout change across a pause"),
    ("missing feed loses fallback requirement", "app-updates", "Sources/AltF4/Services/AppUpdates/AppUpdateFeedSupport.swift",
    ...
    ("current catalog app loses coverage", "app-updates", "Sources/AltF4/Services/AppUpdates/AppUpdatesSupport.swift",
    ...
    ("ambiguous catalog claims coverage", "app-updates", "Sources/AltF4/Services/AppUpdates/AppUpdatesSupport.swift",
    ...
    ("switcher reveal before resize", "switcher", "Sources/AltF4/UI/Switcher/SwitcherView.swift",
    ...
    ("switcher loses replacement identity", "switcher", "Sources/AltF4/UI/Switcher/SwitcherView.swift",
    ...
    ("switcher follows window count", "core", "Sources/AltF4/Services/Switcher/SwitcherSupport.swift",
    ...
    ("invalid saved zoom", "core", "Sources/AltF4/Services/QuickTools/ScreenshotSupport.swift",
    ...
    ("missing recording action", "launcher", "Sources/AltF4/Services/QuickTools/QuickLauncherService.swift",
    ...
    ("incorrect recording icon", "launcher", "Sources/AltF4/UI/QuickLauncher/QuickLauncherView.swift",
    ...
    ("missing translation", "localization", "Sources/AltF4/Core/FeatureStrings.swift",
    ...
    ("capture accepts a refreshing source", "capture",
     "Sources/AltF4/Services/QuickTools/ScreenshotSelectionController.swift",
    ...
    ("overwrite unreadable notes", "storage", "Sources/AltF4/Services/QuickTools/ScratchpadStore.swift",
