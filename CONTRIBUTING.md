...
./build/F4 --selftest       # quick health check (SELFTEST OK)
...
called `F4 Utils Signing` in a dedicated keychain automatically when no
...
once (`tccutil reset Accessibility com.f4.utils.dev`) and grant it
...
is present, with the hardened runtime and `Resources/F4.entitlements`,
...
| Folder | Role |
|---|---|
| `Sources/F4/App` | App lifecycle and the menu bar status item |
| `Sources/F4/Core` | Localization, permissions, UserDefaults keys |
| `Sources/F4/Services` | All behavior, like energy, monitor, scroll and switcher |
| `Sources/F4/UI` | SwiftUI views only, no business logic |
| `Sources/F4/Support` | `--selftest` and `--sensors` diagnostics |
...
F4 ships these locales today: English (US), Português (Brasil),
...
[new issue](https://github.com/AleexCruuz/f4/issues/new/choose) page.

- **Bug report.** Include your F4 version from Settings under About and
...
6. Settle the direction before writing the code. Scope is the most common
   ...
   switch it on. F4 reaches for what macOS almost does; it does not
